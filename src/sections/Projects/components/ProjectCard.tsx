      ...
      <TiltCard className="flex h-full flex-col overflow-hidden rounded-3xl border border-white/10 bg-[color:var(--bg-soft)] shadow-[0_10px_40px_-20px_rgba(0,0,0,0.6)] transition-[border-color,box-shadow] duration-500 group-hover:border-[color:var(--accent)]/40 group-hover:shadow-[0_28px_80px_-24px_rgba(255,77,125,0.35)]">
        <div className="relative aspect-[4/3] overflow-hidden">
          {project.frontImage ? (
            ...
            <div className="flex h-full w-full items-center justify-center bg-[radial-gradient(circle_at_50%_40%,rgba(255,77,125,0.14),transparent_70%)]">
              <span className="font-mono text-xs uppercase tracking-[0.35em] text-white/30">
                {project.title}
              </span>
            </div>
          )}
          ...
          {inDev && (
            <span className="absolute left-5 top-4 flex items-center gap-1.5 rounded-full border border-amber-300/30 bg-amber-400/20 px-2.5 py-1 text-[10px] font-black uppercase tracking-wider text-amber-300">
          ...
          <div className="absolute inset-0 flex items-center justify-center opacity-0 transition-opacity duration-300 group-hover:opacity-100">
            <span className="flex items-center gap-2 rounded-full border border-white/20 bg-black/70 px-5 py-2.5 text-sm font-bold text-white">
